...
> **Provenance rule (do not violate).** This file is a **transfer document**, written in
> the `wyrhta-labs` meta repo (`docs/website-brief.md` there) and copied here. The meta
> repo's `docs/strategy.md` is the source of truth; the website is a **downstream
> rendering** and gets corrected to match — never the reverse. Do not edit this brief
> here: strategy changes go back to the meta repo, which then re-issues the brief. This
> brief summarises `strategy.md`, the ADRs, `IDEAS.md`, and `manual-todo.md` as of the
> date above. If this brief and `strategy.md` ever disagree, `strategy.md` wins.
>
